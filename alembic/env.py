...
def _get_url() -> str:
    ...
    try:
        ...
        cfg = load_config(os.environ.get("FORGERAG_CONFIG"))
        engine = build_engine(cfg.persistence.relational)
        return str(engine.url)
    except Exception:
        # Fallback: try env var
        url = os.environ.get("FORGERAG_DATABASE_URL")
        if url:
            return url
        raise RuntimeError("Cannot resolve database URL. Set FORGERAG_CONFIG or FORGERAG_DATABASE_URL.")
