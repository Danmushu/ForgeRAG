...
def detect_communities(
    ...
    gs = _require_graph(state)
    comm_cfg = state.cfg.graph.community_detection

    # Step 1: Detect communities via Leiden
    resolution = comm_cfg.resolution if comm_cfg else 1.0
    min_size = comm_cfg.min_community_size if comm_cfg else 3
    communities = gs.detect_communities(resolution=resolution)
    communities = [c for c in communities if len(c.entity_ids) >= min_size]

    if not communities:
        return {"communities": 0, "message": "No communities detected"}

    # Step 2: Summarize with LLM (if configured and enabled)
    if comm_cfg and comm_cfg.enabled:
        try:
            from config.auth import resolve_api_key
            from graph.community_summarizer import CommunitySummarizer

            api_key = resolve_api_key(
                api_key=comm_cfg.api_key,
                api_key_env=comm_cfg.api_key_env,
                required=False,
                context="community_summarizer",
            )
            summarizer = CommunitySummarizer(
                model=comm_cfg.model,
                api_key=api_key,
                api_base=comm_cfg.api_base,
            ...
            log.warning("Community summarization failed: %s", e)
    ...
    return {
        ...
        "details": [
            {"id": c.community_id, "size": len(c.entity_ids), "title": c.title, "has_summary": bool(c.summary)}
            for c in communities
        ],
    }
...
def list_communities(state: AppState = Depends(get_state)):
        ...
        "communities": [
            {
                ...
                "title": c.title,
                "summary": c.summary,
                "entity_ids": c.entity_ids[:20],  # truncate for API
            }
            for c in communities
        ],
    }
