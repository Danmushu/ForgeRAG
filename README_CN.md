...
对于**多跳问题**（如 *"苹果和三星的共同供应商有哪些？"*），我们引入**知识图谱**路径：在入库时抽取实体和关系，运行 **Leiden 社区检测**并生成 LLM 摘要，在查询时实现高层主题检索和实体级遍历。KG 路径将**合成的实体描述、关系摘要和社区概览**直接注入生成提示，为 LLM 提供原文分块之上的"蒸馏知识层"。
