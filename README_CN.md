...
对于**多跳问题**（如 *"苹果和三星的共同供应商有哪些？"*），可以在配置页开启**知识图谱社区**功能：在入库时抽取实体和关系，运行 **Leiden 社区检测**并生成 LLM 摘要，在查询时实现高层主题检索和实体级遍历。KG 路径将**合成的实体描述、关系摘要和社区概览**直接注入生成提示，为 LLM 提供原文分块之上的"蒸馏知识层"。
...
> **提示：** 生产环境（Docker/Linux）建议 `--workers 4`。Windows 下因 uvicorn 多进程限制请使用 `--workers 1`。文档入库涉及大量 LLM 调用，多 worker 可保持 UI 响应。
...
| 组件 | 选项 |
|------|------|
| **PDF 解析** | PyMuPDF（快速）→ MinerU（布局感知、表格/公式）→ VLM（视觉语言） |
...
- [LightRAG](https://github.com/HKUDS/LightRAG) — 基于图的 RAG，社区摘要检索
- [GraphRAG](https://github.com/microsoft/graphrag) — 微软的图驱动 RAG
